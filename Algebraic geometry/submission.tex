...
\end{lemma}
I did not include the proof here, as it contained a slight mistake.
...
\begin{proof}
    ...
    \end{equation*}
\end{proof}
Sadly, I have made a crucial mistake in my solution to the next question.
This mistake boils down to the fact that in general
\begin{equation*}
    \extpow^{2d} V \not\cong \extpow^2 \extpow^d V
\end{equation*}
Observe e.g. that $\dim \extpow^4 k^3 = 0$ but $\dim \extpow^2 \extpow^2 k^3 = 3$.
In fact, the next lemma only holds for $d = 2$.
\begin{lemma}[Question (c)]
    \label{prop:characterization_decomposable_d_even}
    Let $d$ be \sout{even} two.
...
\begin{proof}
    ...
    \footnote{Here is the mistake: Here and further down, we consider $v_i \wedge v_j \in \extpow^2 \extpow^d V$, and so the permutation is just $(1 \ 2)$, which clearly has odd parity.
    If we would consider $v_i \wedge v_j \in \extpow^{2d} V$, this step seems to work out, but below, we then cannot apply Lemma~\ref{prop:extpow_zero_iff_independent} and Lemma~\ref{prop:subspace_closed}}.
