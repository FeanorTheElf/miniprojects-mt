...
\begin{proof}
    ...
    \begin{equation*}
        \chi_f(a)\chi_f(b) = f([a])f([b]) = f([ab]) = \chi_f(ab)
    \end{equation*}
    as also $ab \perp q$.
...
\begin{proof}
    ...
    \begin{equation*}
        \sum_{X \leq n \leq Y} \left| \chi(n) n^{-s} \right| \leq \sum_{X \leq n \leq Y} C \left| n^{-s} \right| \leq C \sum_{X \leq n \leq Y} \left| n^{-s} \right|
    \end{equation*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\geq \sum_{n \geq 0} \ \sum_{0 < k \leq q}\sgn(c) \chi(qn + k) (qn + q)^{-s} \\
        &= \sum_{n \geq 0} \sgn(c) (qn + q)^{-s} \underbrace{\sum_{0 < k \leq q} \chi(qn + k)}_{= c} \\
        &= \frac {|c|} {q^s} \sum_{n \geq 1} n^{-s} = \frac {|c|} {(q + 1)^s} \zeta(s)
    \end{align*}
    ...
    \begin{align*}
        ...
        &\leq (qn)^s \frac {sk(qn)^{-1}} {(q^2n^2 + qnk)^s} = \frac {sk} {qn(qn + k)^s} = O(s(qn)^{-s - 1})
    \end{align*}
    If $s > 1$ and $k \leq q$, then also $(qn)^{-s} - (qn + k)^{-s} = O(s(qn)^{-(1 + \epsilon)})$ for some small enough $0 < \epsilon < 1$.
    ...
    \begin{align*}
        ...
        =& \ O(qCX^{-s} + qCY^{-s}) + \sum_{X/q \leq n \leq Y/q} \ \sum_{0 < k \leq q} \chi(qn + k) \Bigl( (qn)^{-s} + \underbrace{(qn + k)^{-s} - (qn)^{-s}}_{= O(s(qn)^{-(1 + \epsilon)})} \Bigr) \\
        =& \ O(qCX^{-s}) + \sum_{X/q \leq n \leq Y/q} \Bigl( (qn)^{-s} c + \sum_{0 < k \leq q} O(Cs(qn)^{-(1 + \epsilon)}) \Bigr) = \\
        =& \ O(qCX^{-s}) + 0 + O\Bigl( Cqs \sum_{X/q \leq n \leq Y/q} (qn)^{-(1 + \epsilon)} \Bigr) \\
        =& \ O(qCX^{-s}) + O\Bigl( Cq^{-\epsilon}s \sum_{n \geq X/q} n^{-(1 + \epsilon)} \Bigr)
    \end{align*}
    ...
    Thus the limit is continuous and extends $L(s, \chi)$ defined on $]1, \infty[$.
    It follows that $\lim_{t \to s^+} L(t, \chi)$ exists and is equal to $\sum_{n \geq 1} \chi(n) n^{-s}$.
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    as by Taylor series expansion, find for $|x| < 1$
    \begin{equation*}
...
For this, it uses again the functions $L(s, \chi)$ as introduced here.
\begin{corollary}[Question (iii)]
...
\begin{corollary}[Question (ii)]
    ...
    \begin{equation*}
        g: \N \to \C, \quad n \mapsto \begin{cases}
            \omega^{\log_{[r]} [n]} & \text{if $n \perp q$} \\
            0 & \text{otherwise}
        \end{cases}
    \end{equation*}
...
\begin{proposition}
    ...
    Then $\mathrm{im}f \leq S_q$ is a subgroup where $S_q := \{ \omega_q^k \ | \ k \in \Z \}$ is the group of $q$-th roots of unity.
...
\begin{proof}
    ...
    Then
    \begin{equation*}
        \sum_{a \in (\Z/q\Z)^\units} \chi(a) = \sum_{k = 1}^{q - 1} \chi(r^k) = \sum_{k = 1}^{q - 1} \xi^k = \xi \frac {1 - \xi^{q - 1}} {q - \xi} = 0 
    \end{equation*}
    ...
    \begin{align*}
        ...
        &= \sum_{\xi \ \text{$q$-th root of unity}} \xi^k = \sum_{l = 1}^{q - 1} \omega^{kl} = \omega^{k} \frac {1 - (\omega^{q - 1})^k} {1 - \omega^k} = 0 
    \end{align*}
    ...
    \begin{equation*}
        ...
        \end{cases}
    \end{equation*}
    The condition $[a][b]^{-1} = 1$ is equivalent to $a \equiv b \pmod q$, so the claim follows.
\end{proof}
...
\begin{corollary}[Question (iv)]
    ...
    Then the function $L(s, \chi)'/L(s, \chi)$ is bounded on a neighborhood of $1$, provided that $L(1, \chi) \neq 0$.
\end{corollary}
\begin{proof}
    ...
    However, the derivative of a holomorphic function is again holomorphic, so $L(s, \chi)'$ has no pole at $s = 1$ (since we know that $L(s, \chi)$ has not pole at $s = 1$ for $\chi \neq \chi_0$).
