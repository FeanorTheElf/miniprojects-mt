...
\begin{proof}
    ...
    \begin{align*}
        \sgn(c) \sum_{n \geq 1} \chi(n) n^{-s} &= \sum_{n \geq 1} \ \sum_{0 \leq k < q} \sgn(c) \chi(qn + k) (qn + k)^{-s} \\
        &\geq \sum_{n \geq 1} \ \sum_{0 \leq k < 1}\sgn(c) \chi(qn + k) (qn + n)^{-s} \\
        &= \sum_{n \geq 1} \sgn(c) (qn + n)^{-s} \underbrace{\sum_{0 \leq k < q} \chi(qn + k)}_{= c} \\
    ...
    \begin{align*}
        ...
        &= \sum_{n \geq 1} \ \sum_{0 \leq k < q} \chi(qn + k) \Bigl( (qn)^{-s} + (qn + k)^{-s} - (qn)^{-s} \Bigr)
    \end{align*}
    ...
    \end{align*}
    As $\chi((\Z/q\Z)^\units) \subseteq \C$ is finite, find $C > 0$ with $|\chi_f(n)| \leq C$ for all $n \in \N$. Then
    \begin{align*}
        \sum_{n \geq X} \chi(n) n^{-s} &= O(qCX^{-s}) + \sum_{n \geq X/q} \ \sum_{0 \leq k < q} \chi(qn + k) \Bigl( (qn)^{-s} + O(sn^{-s - 1}) \Bigr) \\
    ...
    Further, the expression converges uniformly (as a function in $s$ on $\Re(s) \geq \epsilon > 0$) to $0$ as $X \to \infty$. 
    So
    \begin{equation*}
        \sum_{n < X} \chi(n) n^{-s} \quad \text{converges uniformly to} \quad \sum_{n \geq 1} \chi(n) n^{-s}
    \end{equation*}
    as $X \to \infty$ (on a $\Re(s) \geq \epsilon > 0$). 
    Thus the limit is continuous and a continuation of $L(s, \chi_f)$ defined on $\Re(s) > 1$.
    From this it follows that $\lim_{t \to s} L(t, \chi_f)$ exists and is equal to $\sum_n \chi_f(n) n^{-s}$.
\end{proof}
